...
function showMsg(msg) {
    ...
    var content = "";
    var customs = msg.getElems().filter(item => item.getType() == webim.MSG_ELEMENT_TYPE.CUSTOM);
    switch (subType) {
        ...
        case webim.GROUP_MSG_SUB_TYPE.TIP: //群提示消息
            // var opType = msg.getElems()[0].getContent().getOpType();
            // if (opType == 2) {
            //     content = ''
            // } else {
    ...
    for (var i in elems) {
        ...
        switch (type) {
            ...
            case webim.MSG_ELEMENT_TYPE.LOCATION: //暂不支持地理位置
                ...
                break;
            case webim.MSG_ELEMENT_TYPE.CUSTOM: // 附加数据不显示
                // html += convertCustomMsgToHtml(content);
                break;
            case webim.MSG_ELEMENT_TYPE.GROUP_TIP: // 群消息提示
...
function convertGroupTipMsgToHtml(content) {
    ...
    switch (opType) {
        case webim.GROUP_TIP_TYPE.JOIN: //加入群
            text += content.userinfo[0].NickName;
            //text += opUserId + "邀请了";
...
//发送消息(普通消息)
function onSendMsg(msg, customs, callback) {
