  ...
  methods: {
    ...
    _setCalendarData: function(year, month) {
      ...
      for (let i = 1; i <= days_count; i++) {
        ...
        // 当前天标记出来
        if (tmp === today) {
          temp.push(Object.assign(common, { today: true }));
        } else {
          temp.push(common);
        }
      }
      ...
      for (let i = 0; i < this.data.days_color.length; i++) {
        ...
        const is_return_week = item.is_return_week;
        const is_return = item.is_return;
        const id = item.id;
        for (let j = 0; j < days.length; j++) {
          if (days[j].info == item.month && days[j].day == item.day) {
            ...
            if (status) {
              days[j].status = status;
              if (status.indexOf('doctor') > -1) {
                days[j].recheckDay = true;
              }
            }
            if (cycle) {
              days[j].cycle = cycle;
            ...
            if (is_return_week) {
              days[j].is_return_week = is_return_week;
            }
            if (is_return) {
              days[j].is_return = is_return;
            }
            days[j].id = id;
          }
        }
    ...
    dayClick: function(event) {
      ...
      const isSameOrBefore = new Date(year, month, day, '00', '00', '00').valueOf() <= current;
      if (isSameOrBefore) {
        const eventDetail = {
          year,
          month,
    ...
    dayLongPress: function(event) {
      ...
      //   new Date().getDate();
      // // 今天及以后的日子才可以设置
      // var after = moment(time).isAfter(current);
      // var equal = moment(time).isSame(current);
      const time = '' + year + '-' + month + '-' + day;
      const current = new Date(
        new Date().getFullYear(),
        new Date().getMonth() + 1,
        new Date().getDate()).valueOf();
      const isSameOrAfter = new Date(year, month, day).valueOf() >= current;
      if (isSameOrAfter) {
        const eventDetail = {
          year,
          month,
        ...
        this.triggerEvent('dayLongPress', eventDetail);
      }
    }
  },
